...
class KOrderController extends Controller
{
    public function order() {
        $order = Order::all();
        $user = User::where('level_id','5')->get();
    	return view('kasir.master.order.index', compact('order','user'));
    }

    public function tambah() {
        $order = Order::all();
        $user = User::where('level_id','5')->get();
    	return view('kasir.master.order.tambah', compact('order','user'));
    }
    ...
    public function edit($id) {
    	return view('kasir.master.order.edit');
    }

    public function detail($id) {
    	$order = Order::find($id);	
    	return view('kasir.master.order.detail', compact('order'));
    }
    ...
    public function pilihmenu(){
        ...
        return view('kasir.master.order.pilih', compact('masakan','detailorder'));
    }
    ...
    public function transaksi(){
        ...
        $user = User::where('level_id','5')->get();
    	return view('kasir.master.transaksi.index', compact('transaksi','user'));
    }
    ...
    public function transaksi_update($order_id){
        ...
        $masakan = Masakan::All();
        return view('kasir.master.transaksi.detail', compact('masakan','transaksi','order','detailorder','user'));
    ...
    public function transaksi_detail($order_id){ 
        ...
        $detailorder = Detailorder::where('order_id',$order_id)->get();
        $masakan = Masakan::All();
        return view('kasir.master.transaksi.invoice', compact('masakan','transaksi','order','detailorder','user'));
    }
